...
export class Person {
  ...
  isValidAge(value) {
    const result = (typeof value === 'number' && value >= 18 && value <= 60)
    if(result == false) {
      console.log('Not a valid age', value);
    }
    return result;
  }

  isValidGender(value) {
    const result = (typeof value === 'string' && Object.values(GENDERS).indexOf(value) !== -1)
    if(result == false) {
      console.log('Not a valid gender', value);
    }
    return result;
  }
